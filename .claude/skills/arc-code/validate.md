...
- **Redundant single-child wrappers** — an element wrapping a single child (or the sole child of its parent) whose `className` could move onto the parent/child with no visual change (`html.md` § Collapse Single-Child Wrappers). Run the merge-up test in your head from the component's SCSS; a wrapper survives only as a genuine multi-slot layout with a sibling present **now**, a conflicting parent box role, or reuse — never anticipated-but-absent structure. Keyword scans never catch this (it needs the CSS of both elements), so it MUST get a verdict here.
