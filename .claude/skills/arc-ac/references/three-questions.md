...
**Latency vs hard failure are separate scenarios.** Cover both:
- *Hard fail* — request errored, UI shows error state and recovery option
- *Slow success* — request takes longer than expected but eventually succeeds; UI must remain stable (loading state visible, action button disabled, no double-submit)

A single "request failed" scenario is incomplete coverage of Question 1. Slow-success has its own observable contract.
