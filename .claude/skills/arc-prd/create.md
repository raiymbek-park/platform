...
Run validation against the five criteria. Read [validate.md](validate.md) Steps 2–3 and apply them to the just-drafted PRD as an in-line check (no separate report file needed at this stage — the goal is gate-keeping, not auditing).

Any FAIL → return to Step 3 and rewrite. Do not proceed to AC until all five criteria pass. This gate exists because a vague PRD produces interpretive AC; fixing AC is harder than fixing the PRD.
