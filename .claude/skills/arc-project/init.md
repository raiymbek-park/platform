...
**Record every rule file in project-context under `## Project Rules`** as an enumerated list — one bullet per file with its repo-relative path and a one-line summary of what the file covers. This list is the source of truth for downstream Arcana skills (especially `/arc:code validate`) that need to load and apply the rules. A flat comma-separated `Sources` line is NOT acceptable — the enumeration must allow any skill to read project-context and resolve each rule file by path.

Example:
```markdown
## Project Rules

Rule files (enumerated for downstream skills):
- `.claude/rules/coding.md` — naming, arrow-only functions, JSX prop order, async wrapper rules
- `.claude/rules/styling.md` — CSS modules, design tokens, class composition via `pickCss`
- `.claude/rules/html.md` — semantic HTML decision matrix, native element preference
- `.claude/rules/state-boundaries.md` — server / UI / navigation state separation

Adopted: all (compatible/complementary with Arcana methodology)
...
- `{path}` — {one-line summary of what this file covers}
...
- `{path}` — {e.g., biome.json, .editorconfig, .eslintrc}
...
> 6. Conventions
> 7. CI
> 8. Project Rules (re-scan rule files, re-enumerate for downstream skills)
> 9. Required Skills (re-run tooling detection + install)
> 10. Model Assignment
