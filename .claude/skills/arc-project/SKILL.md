...
description: >
  ...
  (install external skills + scan codebase + discover project rules), audit checks
  everything is in place, skill-up analyzes completed tasks and proposes improvements.
  Init records every discovered rule file (path + one-line summary) into
  `.arcana/project-context.md` so all downstream Arcana skills — and especially
  `/arc:code validate` — can find and apply them.
  Use when onboarding Arcana to a new project, checking configuration, or after
  completing a task cycle.
...
```
First time:     /arc:project init            → full onboarding (skills + scan + rule discovery)
...
Changed stack:  /arc:project init --refresh  → rescan codebase + re-discover rules

Added rules:    /arc:project init --refine   → re-run rule discovery section
                                                so /arc:code validate picks them up
...
A core responsibility of `init` is to discover the project's coding rules
(`CLAUDE.md`, `.claude/rules/*.md`, `.cursorrules`, `AGENT.md`, `.editorconfig`,
etc.) and record them in `.arcana/project-context.md` as an enumerated list
with a one-line summary per file. Downstream Arcana skills rely on this
list — especially `/arc:code validate`, which uses it as the source of truth
for what to audit.

If a project ships rule files but `init` did not enumerate them, run
`/arc:project init --refine` and select **Project Rules** to re-scan.
