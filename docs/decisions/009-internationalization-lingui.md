...
- **Cons:** Requires a macro transform at build time. Under Vite's Rolldown pipeline the React plugin no longer exposes a Babel hook, so the macro is wired through a dedicated `@rolldown/plugin-babel` instance with `linguiTransformerBabelPreset` — a small extra build dependency rather than a reuse of the React plugin's own transform.
...
- Relationship with ADR 001 (state boundaries): locale is client-owned UI state, held in `localStorage` and Lingui's own `i18n` runtime (which acts as the reactive store for this one value) rather than in a Zustand store — it sits on the client-truth side of the boundary, not server or navigation state.
