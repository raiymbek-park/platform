...
- **Admin-seeded content** — `service-contacts` stay in their stored language; their translations,
  ...
  events: an event carries a post's or issue's own title, which is translated content and is read
  through this feature's projection like anywhere else (see
  `docs/features/push-notifications/prd.md` and `docs/features/home/prd.md`).
