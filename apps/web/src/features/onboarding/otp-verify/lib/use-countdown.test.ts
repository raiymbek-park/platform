...
// edge S1 — countdown ticks down from resendAvailableAt
...
// edge S1 — growing cooldown durations render correctly
...
// Kill: [resendAvailableAt] → [] dep array mutation
...
// Kill: `if (next === 0) clearInterval(id)` → `if (false)` — interval never stops
// Detectable: after expiry the state must not keep changing beyond 0
